---
title: "Superlearner_Pipeline_Blossom_Anxiety_Simple_Model"
format: html
editor: visual
---
...
## Here we might decide a-priori to filter specific EEG metrics and/or frequency bands
X <- X %>% 
  dplyr::select(-contains("Alpha")) %>%
  dplyr::select(-contains("Beta")) %>%
  dplyr::select(-contains("Gamma")) %>%
  dplyr::select(-contains("Theta")) %>%
  dplyr::select(-contains("Imag_Coherence")) %>%
  dplyr::select(-contains("WPLI")) %>%
  dplyr::select(-contains("WPPC"))
